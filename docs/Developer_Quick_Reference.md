...
function zapAndDeposit(IERC20 tokenIn, uint256 amount, bytes32 depositId, uint256 slippageBps) external payable

// Curator approval function
function approveDeposit(bytes32 depositId, uint256 approvedAmount) external onlyRole(VAULT_CURATOR_ROLE)
...
function approveRedeem(bytes32 redeemId, uint256 usdcAmount) external onlyRole(VAULT_CURATOR_ROLE)
function claimRedeem(bytes32 redeemId) external returns (uint256 usdcAmount)
...
const depositId = ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
        ...
        [userAddress, nonce, amount, block.timestamp],
    ),
);

// 3. Zap and deposit
const tx = await zapper.zapAndDeposit(tokenAddress, amount, depositId, 100); // 1% slippage
const receipt = await tx.wait();

// 4. Get shares from event
...
const depositId = await zapper.zapAndDeposit(tokenAddress, amount, depositId, 100);

// 2. Curator approves (with specific amount)
...
const usdcAmount = await zapper.redeem(sharesToRedeem);

// Asynchronous redemption
const redeemId = await zapper.requestRedeem(shares);
// Curator approves
await zapper.approveRedeem(redeemId, usdcAmount);
// User claims
const finalAmount = await zapper.claimRedeem(redeemId);
```
...
async function main() {
    ...
    const depositRegistered = rc1.logs
        .map((l) => {
            try {
                return hostAdapterHost.interface.parseLog(l);
            } catch {
                return null;
            }
        })
